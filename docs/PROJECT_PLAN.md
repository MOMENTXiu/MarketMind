...
- PostgreSQL/Redis 基础设施：Docker Compose、SQLAlchemy、Alembic、PostgreSQL adapter 骨架。
- 质量门：`make check` 覆盖 backend lint/format/test 和 frontend build；当前基线 `188 passed, 5 skipped`。
...
| 阶段 | 目标 | 交付 |
| --- | --- | --- |
| P1 | 前端测试补强 | API client 单测、polling composable、关键页面状态测试。 |
| P2 | PostgreSQL read/write switch | Provider 配置开关、项目 metadata 双写/读切换、迁移脚本、回滚策略。 |
| P3 | Redis-backed queue | Job queue provider、worker 进程、任务状态一致性、单 worker 限制解除。 |
