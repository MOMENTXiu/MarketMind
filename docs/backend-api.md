...
`run` 成功时返回 HTTP 202。真实后端使用 FastAPI in-process background task，调用方应轮询项目详情直到 `status` 变为 `completed` 或 `failed`。
...
`run` 成功时返回 HTTP 202。真实后端使用 FastAPI in-process background task，调用方应轮询 job 详情直到 `status` 变为 `completed` 或 `failed`。
...
4. 轮询 `GET /api/analysis/projects/{project_id}`。
...
5. 轮询 `GET /api/analysis/jobs/{job_id}?project_id=...`。
...
- 当前分析任务仍是 FastAPI 进程内后台任务，不是 Redis Queue。部署时应使用单 worker；多 worker 或多副本可能导致本地文件状态不可见或覆盖。
- PostgreSQL/Redis 基础设施已经存在，但当前运行时项目状态仍以既有 Provider/Adapter 为准；不要假设所有业务真相都已经写入 PostgreSQL。
