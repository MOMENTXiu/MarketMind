...
from backend.models.project import AnalysisResults, ProjectStatus
...
async def run_project_analysis(project_id: str):
    ...
    try:
        ...
        print(
            f"销售预测完成，R²得分: 销售={prediction_result['data'].get('sales_r2', 0):.4f}, 利润={prediction_result['data'].get('profit_r2', 0):.4f}"
        )
        ...
        clustering_result = await clustering_service.analyze(
            n_clusters=project.parameters.n_clusters, save_path=str(customers_csv_path)
        )
        results.clustering_data = clustering_result.get("data", {})
        results.clustering_data["customers_csv"] = str(customers_csv_path)
        print(
            f"客户聚类完成，共分为 {clustering_result['data'].get('n_clusters', 0)} 个群体，轮廓系数={clustering_result['data'].get('silhouette_score', 0):.4f}"
        )
        ...
        try:
            ...
            if model_result.get("success"):
                print(
                    f"✓ 推荐模型构建完成: {model_result['total_customers']}个客户, {model_result['n_clusters']}个群体, {model_result['n_rules']}条规则"
                )
        ...
        storage.update_project(
            project_id, {"status": ProjectStatus.COMPLETED, "results": results.model_dump()}
        )
        ...
        print(error_msg)
        storage.update_project(
            project_id, {"status": ProjectStatus.FAILED, "error_message": error_msg}
        )
