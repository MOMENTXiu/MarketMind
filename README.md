...
- **本地基础设施**：Docker Compose 提供 PostgreSQL、Redis 与 MinIO；Retail V2 runtime 已通过 Provider Boundary 使用 PostgreSQL state、Redis/RQ queue 与 Redis pub/sub SSE。Redis 只承载队列和事件，不作为业务真相源。
