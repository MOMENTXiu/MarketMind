...
class PredictionService:
    ...
    async def analyze(self, forecast_weeks: int = 13) -> Dict[str, Any]:
        ...
        try:
            ...
            # 6. 生成预测
            forecast_results, summary = self._generate_forecast(weekly_sales, train_data, forecast_weeks)
    ...
    def _remove_outliers(self, data: pd.DataFrame, column: str, factor: float = 1.5) -> pd.DataFrame:
        """使用IQR方法去除异常值"""
    ...
    def _generate_forecast(
        ...
        for i in range(1, forecast_weeks + 1):
            ...
            sales_pred = float(self.scaler_sales_y.inverse_transform(sales_pred_scaled.reshape(-1, 1))[0][0])
            profit_pred = float(self.scaler_profit_y.inverse_transform(profit_pred_scaled.reshape(-1, 1))[0][0])
