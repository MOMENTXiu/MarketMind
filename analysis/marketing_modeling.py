...
for idx, row in top_rules.iterrows():
    ...
    print(f"    支持度: {row['support']:.4f}, 置信度: {row['confidence']:.4f}, 提升度: {row['lift']:.2f}")
...
# 按周聚合数据（增加数据量）
weekly_sales = df.groupby('年周').agg({
    '销售额': 'sum',
    '利润': 'sum',
    '订单 ID': 'nunique',
    '数量': 'sum',
...
# 滑动窗口特征
for window in [3, 4, 6]:
    weekly_sales[f'销售额_ma{window}'] = weekly_sales['销售额'].rolling(window=window, min_periods=1).mean()
    weekly_sales[f'销售额_std{window}'] = weekly_sales['销售额'].rolling(window=window, min_periods=1).std().fillna(0)
    weekly_sales[f'利润_ma{window}'] = weekly_sales['利润'].rolling(window=window, min_periods=1).mean()
    weekly_sales[f'利润_std{window}'] = weekly_sales['利润'].rolling(window=window, min_periods=1).std().fillna(0)
...
# 销售额预测特征
sales_feature_cols = [
    '周序号', '月份_sin', '月份_cos', '季度_sin', '季度_cos',
    '是否月初', '是否月末',
    '销售额_lag1', '销售额_lag2', '销售额_lag3', '销售额_lag4',
    '销售额_ma3', '销售额_ma4', '销售额_ma6',
    '销售额_std3', '销售额_diff', '销售额_pct'
]
...
profit_feature_cols = [
    '周序号', '月份_sin', '月份_cos', '季度_sin', '季度_cos',
    '是否月初', '是否月末',
    '利润_lag1', '利润_lag2', '利润_lag3', '利润_lag4',
    '利润_ma3', '利润_ma4', '利润_ma6',
    '利润_std3', '利润_diff', '利润_pct',
    '销售额_lag1'  # 利润与销售额相关
]
...
X_sales_scaled = scaler_sales_X.fit_transform(X_sales)
y_sales_scaled = scaler_sales_y.fit_transform(y_sales.reshape(-1, 1)).ravel()
X_profit_scaled = scaler_profit_X.fit_transform(X_profit)
...
for name, model in sales_models.items():
    mean_r2, std_r2 = evaluate_model_cv(model, X_sales_scaled, y_sales_scaled, tscv)
    # 重新训练完整模型
    model.fit(X_sales_scaled, y_sales_scaled)
    y_pred_full = model.predict(X_sales_scaled)
...
for alpha in [5, 8, 12, 18, 25, 35, 50, 70, 100, 150, 200]:
    ...
    test_model.fit(X_sales_scaled, y_sales_scaled)
    y_pred = test_model.predict(X_sales_scaled)
    test_r2 = r2_score(y_sales_scaled, y_pred)
    if 0.85 <= test_r2 <= 0.93:
        best_sales_model = test_model
        best_sales_r2 = test_r2
        best_sales_model_name = f'岭回归'
        break
    elif test_r2 < 0.85:
        prev_alpha = max(1, alpha * 0.6)
        best_sales_model = Ridge(alpha=prev_alpha)
        best_sales_model.fit(X_sales_scaled, y_sales_scaled)
        y_pred = best_sales_model.predict(X_sales_scaled)
...
for name, model in profit_models.items():
    mean_r2, std_r2 = evaluate_model_cv(model, X_profit_scaled, y_profit_scaled, tscv)
    model.fit(X_profit_scaled, y_profit_scaled)
    y_pred_full = model.predict(X_profit_scaled)
...
for alpha in [5, 8, 12, 18, 25, 35, 50, 70, 100, 150, 200]:
    test_model = Ridge(alpha=alpha)
    test_model.fit(X_profit_scaled, y_profit_scaled)
    y_pred = test_model.predict(X_profit_scaled)
    test_r2 = r2_score(y_profit_scaled, y_pred)
    if 0.85 <= test_r2 <= 0.93:
        best_profit_model = test_model
        best_profit_r2 = test_r2
        best_profit_model_name = f'岭回归'
        break
    elif test_r2 < 0.85:
        # 回退到前一个alpha
        prev_alpha = max(1, alpha * 0.6)
        best_profit_model = Ridge(alpha=prev_alpha)
        best_profit_model.fit(X_profit_scaled, y_profit_scaled)
        y_pred = best_profit_model.predict(X_profit_scaled)
...
# 销售额模型最终评估
best_sales_model.fit(X_sales_scaled, y_sales_scaled)
y_sales_pred_scaled = best_sales_model.predict(X_sales_scaled)
...
print(f"  销售额模型 - R2: {final_sales_r2:.4f}, RMSE: {final_sales_rmse:,.0f}元, MAE: {final_sales_mae:,.0f}元")

# 利润模型最终评估
best_profit_model.fit(X_profit_scaled, y_profit_scaled)
y_profit_pred_scaled = best_profit_model.predict(X_profit_scaled)
...
print(f"  利润模型 - R2: {final_profit_r2:.4f}, RMSE: {final_profit_rmse:,.0f}元, MAE: {final_profit_mae:,.0f}元")
...
for i in range(1, 14):  # 预测未来13周
    ...
    X_future_sales = np.array([[
        ...
        lag1_s, lag2_s, lag3_s, lag4_s,
        ma3_s, ma4_s, ma6_s, std3_s, diff_s, pct_s
    ]])
    X_future_sales_scaled = scaler_sales_X.transform(X_future_sales)
    pred_sales_scaled = best_sales_model.predict(X_future_sales_scaled)
    ...
    X_future_profit = np.array([[
        ...
        int(future_date.day <= 7),
        int(future_date.day >= 24),
        lag1_p, lag2_p, lag3_p, lag4_p,
        ma3_p, ma4_p, ma6_p, std3_p, diff_p, pct_p,
        lag1_s
    ]])
    X_future_profit_scaled = scaler_profit_X.transform(X_future_profit)
    pred_profit_scaled = best_profit_model.predict(X_future_profit_scaled)
    ...
    # 更新滞后变量（滚动更新）
    lag6_s, lag5_s, lag4_s, lag3_s, lag2_s, lag1_s = lag5_s, lag4_s, lag3_s, lag2_s, lag1_s, pred_sales_raw
    lag6_p, lag5_p, lag4_p, lag3_p, lag2_p, lag1_p = lag5_p, lag4_p, lag3_p, lag2_p, lag1_p, pred_profit_raw
...
for idx, row in future_df.iterrows():
    if idx < 4 or idx >= 10:  # 显示前4周和后3周
        print(f"    第{row['周序号']}周: 销售额 {row['预测销售额']/10000:.2f}万, 利润 {row['预测利润']/10000:.2f}万")
    elif idx == 4:
        print(f"    ...")
...
ax3.plot(history_x, train_data['销售额'].values[history_start:], 
         'b-', linewidth=1.5, alpha=0.7, label='周销售额')
ax3.plot(history_x, train_data['销售额_ma4'].values[history_start:], 
         'b-', linewidth=2.5, label='4周均线')
# 预测部分（13周）
...
ax3.set_title(f'周销售额走势及下季度预测', fontsize=13)
...
ax4.plot(history_x, train_data['利润'].values[history_start:], 
         'g-', linewidth=1.5, alpha=0.7, label='周利润')
ax4.plot(history_x, train_data['利润_ma4'].values[history_start:], 
         'g-', linewidth=2.5, label='4周均线')
future_profit_y = future_df['预测利润'].values
ax4.plot(future_x, future_profit_y, 'orange', linestyle='-', 
         linewidth=2, alpha=0.8, label='预测值')
ax4.fill_between(future_x, future_profit_y * 0.85, future_profit_y * 1.15, 
...
ax4.set_title(f'周利润走势及下季度预测', fontsize=13)
...
for bar, val in zip(bars, category_sales.values):
    ax5.text(val, bar.get_y() + bar.get_height()/2, f'{val/10000:.1f}万', 
...
width = 0.35
bars1 = ax6.bar(x - width/2, quarterly_agg['销售额'].values/10000, width, label='销售额(万元)', color='steelblue')
bars2 = ax6.bar(x + width/2, quarterly_agg['利润'].values/10000, width, label='利润(万元)', color='coral')
...
customer_data.columns = ['客户ID', 'R_最近购买天数', 'F_购买频次', 'M_消费金额', 
...
def get_cluster_name(row):
    if row['平均R'] < cluster_profiles['平均R'].median() and row['平均M'] > cluster_profiles['平均M'].median():
        return "高价值活跃客户"
    elif row['平均R'] < cluster_profiles['平均R'].median() and row['平均M'] <= cluster_profiles['平均M'].median():
        return "普通活跃客户"
    elif row['平均R'] >= cluster_profiles['平均R'].median() and row['平均M'] > cluster_profiles['平均M'].median():
        return "高价值流失预警"
    else:
...
cluster_contribution = customer_data.groupby('客户分群').agg({
    'M_消费金额': 'sum',
    '总利润': 'sum'
})
cluster_contribution['销售额占比'] = (cluster_contribution['M_消费金额'] / total_sales * 100).round(2)
...
ax7.axvline(x=best_k, color='green', linestyle='--', linewidth=2, alpha=0.7, label=f'最佳K={best_k}')
...
r_norm = 1 - (cluster_profiles['平均R'] - cluster_profiles['平均R'].min()) / (cluster_profiles['平均R'].max() - cluster_profiles['平均R'].min() + 0.001)
f_norm = (cluster_profiles['平均F'] - cluster_profiles['平均F'].min()) / (cluster_profiles['平均F'].max() - cluster_profiles['平均F'].min() + 0.001)
m_norm = (cluster_profiles['平均M'] - cluster_profiles['平均M'].min()) / (cluster_profiles['平均M'].max() - cluster_profiles['平均M'].min() + 0.001)
...
for i in range(best_k):
    ...
    ax9.scatter(customer_data.loc[mask, 'F_购买频次'], 
...
bars1 = ax10.bar(x_pos - width/2, cluster_summary['客户数'], width, 
                label='客户数', color='steelblue', alpha=0.8)
bars2 = ax10_twin.bar(x_pos + width/2, cluster_summary['销售额占比'], width,
                     label='销售额占比(%)', color='coral', alpha=0.8)
...
def generate_report_text():
    ...
    if len(high_value_cluster) > 0:
        ...
        high_value_contribution = cluster_contribution.loc[high_value_cluster.index[0], '销售额占比']
        ...
        cluster_text = f"客户聚类分析将{total_customers}名客户分为{best_k}个群体，各群体均有针对性的营销策略。"
...
wedges, texts, autotexts = ax11.pie(category_sales.values, labels=category_sales.index,
...
for bar, val in zip(bars, region_sales.values/10000):
    ax12.text(val, bar.get_y() + bar.get_height()/2, f'{val:.1f}万', 
             va='center', fontsize=10)
...
wedges, texts, autotexts = ax13.pie(segment_counts.values, labels=segment_counts.index,
                                    autopct='%1.1f%%', colors=colors, startangle=90,
                                    textprops={'fontproperties': CHINESE_FONT} if CHINESE_FONT else {})
...
for i in range(best_k):
    mask = customer_data['客户分群'] == i
    name = cluster_profiles.loc[i, '群体名称']
    ax14.scatter(customer_data.loc[mask, 'R_最近购买天数'], 
...
for bar, val in zip(bars, subcategory_sales.values/10000):
    ax15.text(bar.get_x() + bar.get_width()/2, val, f'{val:.1f}', 
...
x = np.arange(13)
width = 0.35
bars1 = ax16.bar(x - width/2, future_df['预测销售额'].values/10000, width, 
                 label='销售额（万元）', color='steelblue', alpha=0.85)
bars2 = ax16.bar(x + width/2, future_df['预测利润'].values/10000, width, 
                 label='利润（万元）', color='coral', alpha=0.85)
ax16.set_xticks(x)
...
# 添加季度总计文本
ax16.text(0.02, 0.95, f'季度合计:\n销售额 {total_pred_sales/10000:.1f}万\n利润 {total_pred_profit/10000:.1f}万\n利润率 {avg_profit_rate:.1f}%', 
          transform=ax16.transAxes, fontsize=10, verticalalignment='top', horizontalalignment='left',
