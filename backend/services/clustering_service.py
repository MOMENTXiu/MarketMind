...
class ClusteringService:
    ...
    async def analyze(self, n_clusters: int = 4, save_path: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 5. 确定最佳聚类数（如果需要）
            if n_clusters == 0:
                best_k, _silhouette_scores = self._find_optimal_k(X_scaled)
            else:
            ...
            return {"success": False, "message": f"聚类分析失败: {str(e)}", "data": {}}
    ...
    def _analyze_cluster_rules(
        self, customer_data: pd.DataFrame, df: pd.DataFrame
        ...
        for cluster_id in customer_data["客户分群"].unique():
            # 获取该聚类的客户ID列表
            cluster_customer_ids = customer_data[customer_data["客户分群"] == cluster_id][
                "客户ID"
            ].tolist()
