...
- Data-processing pipeline (`原始数据上传 -> regularization -> analysis2 -> outputs`) is **implemented** in backend runtime. It lives alongside Retail V2; both share the provider/adapter boundary. The design and checklist remain in `docs/architecture/data-processing-pipeline-integration-design.md` and `docs/architecture/data-processing-pipeline-integration-checklist.md` for reference.
